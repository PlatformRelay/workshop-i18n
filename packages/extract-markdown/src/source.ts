 ...
 *
 * This module is a deliberate copy of `@workshop-i18n/extract-slidev`'s `source.ts`.
 * The two extractors are separate published packages that must not depend on each
 * other, and the shared home for it is `@workshop-i18n/core`; promoting it there is a
 * follow-up that belongs in core's own change, not in an extractor's.
 */
