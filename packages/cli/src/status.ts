...
function humanReport(report: StateReport, evaluation: PolicyEvaluation | undefined): string {
  const lines: string[] = []
  ...
  if (evaluation !== undefined) {
    if (evaluation.satisfied) {
      lines.push(`policy ${evaluation.policy}: passed`)
    } else {
      lines.push(`policy ${evaluation.policy}: FAILED`)
...
function selectedLocale(plan: ExtractPlan, requested: unknown): string | undefined {
  ...
  const targets = plan.workspace.manifest.locales.targets
...
function execute(context: CommandContext): number {
  ...
  const only = selectedLocale(plan, values.locale)
  const locales = plan.plans.filter((locale) => only === undefined || locale.locale === only)

  for (const warning of plan.warnings) io.stderr(`${warning}\n`)
  const stale = locales.flatMap((locale) => locale.catalogs.filter(isChanged))
  if (stale.length > 0) {
    io.stderr(
  ...
  const statuses = locales.flatMap((locale) => unitStatuses(plan, locale))
  ...
  if (values.json === true) {
    io.stdout(
      `${JSON.stringify(jsonDocument(plan, report, evaluation, stale.length === 0), null, 2)}\n`,
    )
  } else {
    io.stdout(humanReport(report, evaluation))
  }
  return evaluation === undefined || evaluation.satisfied ? EXIT.OK : EXIT.FAILED
}
