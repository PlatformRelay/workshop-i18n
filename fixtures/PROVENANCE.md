...
| `crlf-and-bom.md` | CRLF line endings and a leading byte-order mark. **This fixture found a real bug**: micromark's preprocessor drops a leading U+FEFF, so every offset in a BOM file was one code unit short — invisible to a losslessness assertion, because untranslated holes copy the original bytes either way |
